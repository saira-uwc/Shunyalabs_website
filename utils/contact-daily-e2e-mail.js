...
export async function runContactDailyE2eMail({
  ...
  // Live site API still enforces one automation send per ~24h (HTTP 429).
  // Soft-pass so a once-daily schedule stays green if mail already went out earlier today.
  // Removing the server limit requires a website/backend change — not controllable here.
