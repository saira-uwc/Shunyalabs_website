...
function saveToHistory(results, playwrightRun) {
  ...
  // Keep only runs from the last 15 days
  const fifteenDaysAgo = new Date(Date.now() - 15 * 24 * 60 * 60 * 1000).toISOString();
  const recentHistory = history.filter(run => run.runDate >= fifteenDaysAgo);
...
<body>
  ...
  <footer>
    ...
    <p style="margin-top: 8px; font-size: 11px;">Total runs stored: ${history.length} | History retention: Last 15 days</p>
  </footer>
  ...
  <script>
    ...
    function renderCalendar() {
      ...
      tab.innerHTML =
        ...
        '<div class="calendar-footer">Total runs stored: ' + historyData.length + ' | History retention: Last 15 days</div>';
    }
